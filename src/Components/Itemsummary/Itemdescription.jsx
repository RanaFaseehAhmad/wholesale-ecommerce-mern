...
function Itemdescription({ item }) {
    ...
    const averageRating =
        item?.reviews?.length
            ? item.reviews.reduce((sum, rev) => sum + rev.rating, 0) /
            item.reviews.length
            : 0;
            ...
            <div className={style.blockDescription}>
                ...
                <div className={style.description}>
                    ...
                    <div className={style.ratingreviewsWrapper}>
                        <TabView >
                            <TabPanel header={`Product reviews (${item?.reviews?.length || 0})`}>
                                    ...
                                    {item?.reviews?.length > 0 ?
                                        (<div className={style.reviewWrapper}>
                                            <div className={style.overallAvgTitleWrapper}>
                                                ...
                                                <p className={style.reviewsNumber}>Based on {item.reviews.length} reviews for <span className={style.varifiedpurchaseText}>Verified Purschase</span> <i className={`pi pi-verified ${style.varifiedpurchaseTextIcon}`}></i></p>
                                            </div>
