...
function ItemRecommended({ productSubcategoryId }) {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`/products/recommendedItems/${productSubcategoryId}`)
          ...
          <div className={style.sliderWrapper}>
            <div className={style.itemBox} ref={menuRef}>
              ...
              {recommendedItems.map((item) => (
                <div key={item._id} onClick={() => selectItem(item._id)} className={style.card}>
                  <div className={style.imgWrapper}>
                    <img src={item.image} alt="item image" />
                  </div>
                  <div className={style.info}>
                    <h3 className={style.itemName}>{item.productName}</h3>
